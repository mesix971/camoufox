...
> 📘 Pour la documentation upstream en anglais, voir [README.md](README.md).
...
Déjà fait ✅ :
- Fingerprint C++ (fpgen)
- Pool de proxies avec rotation
- Captcha solving (CapSolver + 2Captcha)
- Queue detection (6 systèmes)
- Task queue persistante
- Webhook Discord
- Warmup, auto-refresh, rate limiting, cookie persistence
- Retry exponentiel avec jitter
- Discord bridge daemon
- Leak test page
- UI française complète

À faire 🔜 :
- Dashboard métriques temps-réel (CPU/RAM par session)
- CreepJS pipeline automatisé (scoring auto)
- Ghost cursor (mouvements souris en courbes de Bézier)
- Macro recorder (codegen Playwright wrapper)
- Action DSL (YAML → séquence d'actions)
- Canvas/WebGL pixel noise (nécessite patches Firefox supplémentaires)
